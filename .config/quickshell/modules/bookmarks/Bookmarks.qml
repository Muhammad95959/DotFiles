...
Scope {
  ...
  function open(){ visible=true; _query=""; selectedIndex=0; refresh() }
    ...
    PanelWindow{
      ...
      Rectangle{
        ...
        OnelinerBar{
          ...
          onAccepted: (idx, query)=>{ const eff= idx>=0?idx:root.selectedIndex; if(root.filtered.length>0) root.activateAt(eff)}
