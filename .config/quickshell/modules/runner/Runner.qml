...
Scope {
  ...
  function runRaw(cmd) {
    if (!cmd || cmd.trim() === "")
      return
    Quickshell.execDetached(["sh", "-c", cmd.trim()])
    close()
  }
  function activateAt(idx) {
    const q = query.trim()
    const list = filtered
    if (idx >= 0 && idx < list.length && !q.includes(" ")) {
      Quickshell.execDetached([list[idx].path])
      close()
      return
    }
    if (q !== "")
  ...
  function pageMove(dir) { _markKeyboard(); const n = filtered.length; if (n === 0) return; let ni = selectedIndex + dir * root.rowsVisible; if (ni < 0) ni = 0; if (ni >= n) ni = n - 1; selectedIndex = ni }
  ...
  Process {
    ...
    stdout: StdioCollector { waitForEnd: true; onStreamFinished: {
      ...
      for (let line of lines) {
        ...
        let name = "", path = ""
        if (tab >= 0) { name = line.slice(0, tab).trim(); path = line.slice(tab + 1).trim() }
        else { name = line; path = line }
        if (!name)
          continue
  ...
  LazyLoader {
    ...
    Variants {
      ...
      PanelWindow {
        ...
        WlrLayershell.layer: WlrLayer.Overlay; WlrLayershell.keyboardFocus: WlrKeyboardFocus.Exclusive; WlrLayershell.namespace: "quickshell-runner"
        ...
        Rectangle {
          ...
          ColumnLayout {
            ...
            RowLayout { Layout.fillWidth: true; spacing: 10
              ...
              ColumnLayout { spacing: 2; Text { text: "Runner"; color: Theme.fg; font.family: Theme.monoFont; font.pixelSize: 14; font.bold: true } Text { text: root.filtered.length + " / " + root.allEntries.length + " entries"; color: Theme.fg; opacity: 0.55; font.family: Theme.monoFont; font.pixelSize: 11 } }
            ...
            Rectangle {
              ...
              RowLayout { anchors.fill: parent; anchors.leftMargin: 12; anchors.rightMargin: 12; spacing: 8
                ...
                TextInput {
                  ...
                  Keys.onPressed: event => { if (event.key === Qt.Key_Escape) { root.close(); event.accepted = true } else if (event.key === Qt.Key_Backtab) { root.move(-1); event.accepted = true } else if (event.key === Qt.Key_Tab) { if (event.modifiers & Qt.ShiftModifier) root.move(-1); else root.move(1); event.accepted = true } else if (event.key === Qt.Key_Up) { root.moveNoWrap(-1); event.accepted = true } else if (event.key === Qt.Key_Down) { root.moveNoWrap(1); event.accepted = true } else if (event.key === Qt.Key_Left) { root.moveNoWrap(-1); event.accepted = true } else if (event.key === Qt.Key_Right) { root.moveNoWrap(1); event.accepted = true } else if (event.key === Qt.Key_PageUp) { root.pageMove(-1); event.accepted = true } else if (event.key === Qt.Key_PageDown) { root.pageMove(1); event.accepted = true } else if (event.key === Qt.Key_Return || event.key === Qt.Key_Enter) { root.activateAt(root.selectedIndex); event.accepted = true } }
                  Text { anchors.left: parent.left; anchors.verticalCenter: parent.verticalCenter; visible: searchField.text === ""; text: "Type a command…"; color: Theme.fg; opacity: 0.45; font.family: Theme.monoFont; font.pixelSize: 14 }
                }
            ...
            ListView {
              ...
              delegate: Rectangle {
                ...
                ColumnLayout { anchors.fill: parent; anchors.leftMargin: 12; anchors.rightMargin: 12; anchors.topMargin: 6; anchors.bottomMargin: 6; spacing: 2
                  Text { text: del.modelData.name; color: Theme.fg; font.family: Theme.monoFont; font.pixelSize: 12; font.bold: root.selectedIndex === del.index; elide: Text.ElideRight; Layout.fillWidth: true; horizontalAlignment: Text.AlignLeft; LayoutMirroring.enabled: false }
                  Text { text: del.modelData.path; color: Theme.fg; opacity: 0.55; font.family: Theme.monoFont; font.pixelSize: 10; elide: Text.ElideMiddle; Layout.fillWidth: true; horizontalAlignment: Text.AlignLeft; LayoutMirroring.enabled: false }
                }
              ...
              Text { anchors.centerIn: parent; visible: root.filtered.length === 0; text: root.allEntries.length === 0 ? "No executables found" : "Press Enter to run: " + root.query; color: Theme.fg; opacity: 0.55; font.family: Theme.monoFont; font.pixelSize: 13 }
            }
            RowLayout { Layout.alignment: Qt.AlignHCenter; spacing: 10; Text { text: "↵ Run"; color: Theme.fg; opacity: 0.85; font.family: Theme.monoFont; font.pixelSize: 10; font.bold: true } Rectangle { width: 1; height: 10; color: Theme.border; opacity: 0.6 } Text { text: "Esc Close"; color: Theme.fg; opacity: 0.85; font.family: Theme.monoFont; font.pixelSize: 10; font.bold: true } }
          }
  ...
  GlobalShortcut { name: "runnerToggle"; description: "Toggle runner"; onPressed: root.toggle() }
}
