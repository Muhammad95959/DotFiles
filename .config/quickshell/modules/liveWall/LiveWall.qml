...
Scope {
  ...
  function refresh() { _accum = ""; allFiles = []; scanProc.running = true }
  function activateAt(idx) {
    const list = filtered
    if (idx < 0 || idx >= list.length)
      return
    LiveWallManager.setVideo(list[idx], true)
    close()
  }
  function toggleLive() { LiveWallManager.toggleLive() }
  ...
  Process {
    ...
    onExited: {
      root.allFiles = root._accum.split("\n").map(s => s.trim()).filter(s => s.length > 0)
