...
Scope {
  ...
  function activateAt(idx) {
    ...
    if (idx < 0 || idx >= list.length)
      return
    const safe = list[idx].name.replace(/'/g, "'\\''")
  ...
  Process {
    ...
    onExited: {
      ...
      for (let line of lines) {
        ...
        if (!n)
          continue
        out.push({ name: n, color: c || Theme.accent })
      }
      root.allEntries = out
            ...
            ListView {
              ...
              delegate: Rectangle {
                ...
                RowLayout {
                  ...
                  Rectangle { width: 18; height: 18; radius: 9; color: del.modelData.color; border.color: Qt.alpha(Theme.fg, 0.25); border.width: 1 }
