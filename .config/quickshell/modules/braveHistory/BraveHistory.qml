...
Scope {
  ...
  function activateAt(idx) {
    ...
    if (idx < 0 || idx >= list.length)
      return
    Quickshell.execDetached(["brave-origin", "--test-type", "--password-store=basic", list[idx].url])
    close()
  }
  ...
  Process {
    id: proc
    command: ["sh", "-c", "hist=\"$HOME/.config/BraveSoftware/Brave-Origin/Default/History\"; tmp=\"/tmp/BraveHistory\"; cp --reflink=auto \"$hist\" \"$tmp\" 2>/dev/null || cp \"$hist\" \"$tmp\" 2>/dev/null; cp \"${hist}-journal\" \"${tmp}-journal\" 2>/dev/null; sqlite3 -separator \"  󰛂  \" \"$tmp\" \"SELECT title, url FROM urls WHERE url NOT LIKE 'file://%' ORDER BY last_visit_time DESC;\" 2>/dev/null"]
    stdout: StdioCollector { waitForEnd: true; onStreamFinished: {
      ...
      for (let line of lines) {
        ...
        let title = "", url = ""
        if (idx >= 0) { title = line.slice(0, idx).trim(); url = line.slice(idx + sep.length).trim() }
        else { title = line; url = line }
        ...
        Rectangle {
          ...
          ColumnLayout {
            ...
            RowLayout { Layout.fillWidth: true; spacing: 10
              ...
              ColumnLayout { spacing: 2; Text { text: "Brave History"; color: Theme.fg; font.family: Theme.monoFont; font.pixelSize: 14; font.bold: true } Text { text: root.filtered.length + " / " + root.allEntries.length + " entries"; color: Theme.fg; opacity: 0.55; font.family: Theme.monoFont; font.pixelSize: 11 } }
  ...
  GlobalShortcut { name: "braveHistoryToggle"; description: "Toggle brave history"; onPressed: root.toggle() }
}
